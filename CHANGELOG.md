...
### Added
- `install.sh`: cron (cronie) prompt — installs the package and enables `cron.service`. Tumbleweed doesn't ship with cronie enabled by default; without it, scheduled tasks silently never run.
